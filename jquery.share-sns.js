...
(function ($, window, document) {
    ...
    $.extend(Plugin.prototype, {
        ...
        // イベントハンドラ：Twitter
        clickTwitter: function clickTwitter(e) {
            e.preventDefault();

            // ハッシュタグとアカウントは設定がなければパラメタとして渡さない
            var hashtag = this.data.twHashTag ? '&hashtags=' + this.data.twHashTag : '';
            var account = this.data.twAccount ? '&via=' + this.data.twAccount : '';

            var url = 'https://twitter.com/share?text=' + this.data.title + '&url=' + this.data.url + hashtag + account;
            this.openUrl(url);
        },

        // イベントハンドラ：Facebook
        clickFacebook: function clickFacebook(e) {
            e.preventDefault();

            var url = 'https://www.facebook.com/sharer.php?u=' + this.data.url + '&t=' + this.data.title;
            this.openUrl(url);
        },

        // イベントハンドラ：LINE
        clickLine: function clickLine(e) {
            e.preventDefault();
        ...
        // イベントハンドラ：GooglePlus
        clickGooglePlus: function clickGooglePlus(e) {
            e.preventDefault();
